...
public interface DepartmentRepository extends JpaRepository<Department, String> {
    ...
    @Query("SELECT new com.enfos.reporting.dto.DepartmentDto(d.id, d.name, d.managerName, COUNT(u.id), d.location) " +
            "FROM Department d LEFT JOIN User u ON u.department = d " +
            "GROUP BY d.id, d.name, d.managerName, d.location " +
            "ORDER BY d.id")
